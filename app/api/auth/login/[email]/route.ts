...
export async function POST(req: any, {params}: {params: any}) {
  ...
  if (password.password !== cryptr.decrypt(hash)) {
    ...
    // credentials.name = admins[0].name
    // credentials.email = admins[0].email
    // credentials.id = admins[0].id
    // credentials.role = admins[0].role
    // credentials.token = token

    return NextResponse.json({message: "success"}, {status: 200})
    // return NextResponse.json({name: admins[0].name, email: admins[0].email, id: admins[0].id, role: admins[0].role, token: token}, {status: 200})
  }
